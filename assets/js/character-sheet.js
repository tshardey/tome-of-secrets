document.addEventListener('DOMContentLoaded', function() {
    ...
    const renderActiveAssignments = () => {
        ...
        activeAssignments.forEach((quest, index) => {
            ...
            row.innerHTML = `<td>${quest.type}</td><td>${quest.prompt}</td><td>${quest.book}</td><td>${quest.notes}</td><td class="no-print"><button class="delete-btn" data-index="${index}" data-list="active">Delete</button></td>`;
        });
    ...
    const renderCompletedQuests = () => {
        ...
        completedQuests.forEach((quest, index) => {
            const row = tbody.insertRow();
            row.innerHTML = `<td>${quest.type}</td><td>${quest.prompt}</td><td>${quest.book}</td><td>${quest.notes}</td><td class="no-print"><button class="delete-btn" data-index="${index}" data-list="completed">Delete</button></td>`;
        });
    ...
    document.getElementById('add-item-button').addEventListener('click', () => {
        ...
        if (itemName && allItems[itemName]) {
            inventoryItems.push({ name: itemName, ...allItems[itemName] });
            renderLoadout();
            saveData();
        }
    });
    ...
    addQuestButton.addEventListener('click', () => {
        ...
        if (status === 'active') { activeAssignments.push(newQuest); renderActiveAssignments(); } 
        else { completedQuests.push(newQuest); renderCompletedQuests(); }
        saveData();
    ...
    form.addEventListener('submit', (e) => { e.preventDefault(); saveData(); alert('Character sheet saved!'); });
    ...
    document.querySelector('main').addEventListener('click', (e) => {
        ...
        if (target.classList.contains('delete-ability-btn')) {
            ...
            if (confirm(`Are you sure you want to forget "${abilityName}"? This will refund ${masteryAbilities[abilityName].cost} SMP.`)) {
                let currentSmp = parseInt(smpInput.value, 10) || 0;
                smpInput.value = currentSmp + masteryAbilities[abilityName].cost;
            ...
            if (list === 'active') { activeAssignments.splice(index, 1); renderActiveAssignments(); } 
            else if (list === 'completed') { completedQuests.splice(index, 1); renderCompletedQuests(); }
            saveData();
        }
    });
