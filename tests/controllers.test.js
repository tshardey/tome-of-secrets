...
import { DataExportController } from '../assets/js/controllers/DataExportController.js';
...
describe('Controllers', () => {
    ...
    describe('DataExportController', () => {
        ...
        it('should initialize and attach download/upload handlers', () => {
            const controller = new DataExportController(stateAdapter, form, dependencies);
            controller.initialize();
        ...
        it('should download character data as JSON', () => {
            // Set up some test data
            safeSetJSON(STORAGE_KEYS.CHARACTER_SHEET_FORM, { keeperName: 'Test Keeper', level: '5' });
            safeSetJSON(STORAGE_KEYS.LEARNED_ABILITIES, ['Ability 1', 'Ability 2']);
            
            const keeperNameInput = document.getElementById('keeperName');
            const levelInput = document.getElementById('level');
            if (keeperNameInput) keeperNameInput.value = 'Test Keeper';
        ...
        it('should handle file upload and import data', async () => {
            // Create mock file
            const mockFile = new File(
                [JSON.stringify({
                    version: 1,
                    exportDate: new Date().toISOString(),
                    formData: { keeperName: 'Imported Keeper', level: '10' },
                    characterState: {
                        [STORAGE_KEYS.LEARNED_ABILITIES]: ['Imported Ability']
                    }
                })],
                'test-character.json',
        ...
        it('should reject files missing required structure', async () => {
            const mockFile = new File(
            ...
            const mockFileReader = {
                readAsText: jest.fn(function() {
                    setTimeout(() => {
                        this.onload({
                            target: {
                                result: JSON.stringify({ invalid: 'structure' })
                            }
                        });
                    }, 0);
                }),
